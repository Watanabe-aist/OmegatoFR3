...
def main():
    ...
    try:
        ...
        # ================================================================
        #  ★★ ここから：目標位置に到達した後の測定ループ ★★
        # ================================================================
        print("\n=== Target position reached. Start measuring external forces... ===")
        ...
        try:
            while True:
                ...
                # 外力取得
                F_base = np.array(robot_state.O_F_ext_hat_K)
                F_ee   = np.array(robot_state.K_F_ext_hat_K)

                log_time.append(time.time())
                log_F_base.append(F_base)
                log_F_ee.append(F_ee)

                # 表示（必要ならコメントアウト）
                print(f"EEF force: {F_ee}")
                ...
                # 目標位置は q_goal（または current_position）として固定
                position_error = q - current_position

                tau_task = -np.array(joint_stiffness) * position_error \
                           - np.array(joint_damping) * dq
    ...
    # ========================================================
    # ★★ ここに CSV 保存 & matplotlib プロットを入れる ★★
    # ========================================================
    import csv
    with open("wrench_log.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["time",
                         "Fx_base","Fy_base","Fz_base","Tx_base","Ty_base","Tz_base",
                         "Fx_ee","Fy_ee","Fz_ee","Tx_ee","Ty_ee","Tz_ee"])
        for t, Fb, Fe in zip(log_time, log_F_base, log_F_ee):
            writer.writerow([t, *Fb, *Fe])

    print("Saved wrench_log.csv")

    # ---- matplotlib ----
    import matplotlib.pyplot as plt
    t_arr = np.array(log_time) - log_time[0]
    F_arr = np.array(log_F_ee)

    plt.figure(figsize=(10,6))
    plt.plot(t_arr, F_arr[:,0], label="Fx")
    plt.plot(t_arr, F_arr[:,1], label="Fy")
    plt.plot(t_arr, F_arr[:,2], label="Fz")# typo if necessary: F_arr[:,2]
    plt.legend()
    plt.ylabel("Force [N]")
    plt.xlabel("Time [s]")
    plt.title("EEF External Force")
    plt.grid()
    plt.show()
    # ========================================================
