...
#         self.declare_parameter('vision_data_dir', "")
#         self.declare_parameter('force_data_dir', "")

#         self.vision_data_dir = Path(self.get_parameter('vision_data_dir').value)
#         self.force_data_dir = Path(self.get_parameter('force_data_dir').value)

#         self.force_mode = False
#         self.current_mode = "vision"

#         self.vision_policy = self.init_policy(self.vision_data_dir)
#         self.force_policy = self.init_policy(self.force_data_dir)
...
#         # 変更: policyごとに履歴を分ける
#         self.vision_act_history = deque(maxlen=self.pred_horizon)
#         self.force_act_history = deque(maxlen=self.pred_horizon)
...
#     def init_policy(self, policy_dir):
#         with open(policy_dir / "agent_config.yaml", "r") as f:
#             config_yaml = f.read()
#             policy_config = yaml.safe_load(config_yaml)

#         policy = hydra.utils.instantiate(policy_config)
#         policy_path = policy_dir / "latest_ckpt.ckpt"
#         load_state_dict = torch.load(policy_path, map_location="cpu")
#         policy.load_state_dict(load_state_dict["model"])
...
#             f"Loaded policy from {policy_path} with training step = {train_step}."
#         )
#         return policy

#     # 変更: historyを引数で受け取る
#     def action_chunk_ensemble(self, act_history):
...
#     # 変更: 両方のpolicyを毎回計算し、送る方だけ選ぶ
#     def get_action(self, img_obs, state_obs):
#         with torch.no_grad():
#             vision_pred = self.vision_policy.get_actions(img_obs, state_obs)
#             vision_pred = vision_pred.detach().cpu().numpy()[0]

#             force_pred = self.force_policy.get_actions(img_obs, state_obs)
#             force_pred = force_pred.detach().cpu().numpy()[0]

#         self.vision_act_history.append(vision_pred)
#         self.force_act_history.append(force_pred)

#         vision_cmd = self.action_chunk_ensemble(self.vision_act_history)
#         force_cmd = self.action_chunk_ensemble(self.force_act_history)

#         if self.force_mode:
#             self.current_mode = "force"
#             selected_pred = force_pred
#             selected_cmd = force_cmd
#         else:
#             self.current_mode = "vision"
#             selected_pred = vision_pred
#             selected_cmd = vision_cmd

#         return selected_pred, selected_cmd, vision_pred, force_pred, vision_cmd, force_cmd
...
#         pred_action, cmd_action, vision_pred, force_pred, vision_cmd, force_cmd = self.get_action(
#             img_obs,
#             state_obs
#         )
...
#             self.data_dict["pred_actions"].append(pred_action)
#             self.data_dict["cmd_actions"].append(action_dict)
#             self.data_dict["mode"].append(self.current_mode)

#             # 比較用に両policyの出力も保存
#             self.data_dict["vision_pred_actions"].append(vision_pred)
#             self.data_dict["force_pred_actions"].append(force_pred)
#             self.data_dict["vision_cmd_actions"].append(vision_cmd)
#             self.data_dict["force_cmd_actions"].append(force_cmd)
...
#                 if self.force_mode:
#                     self.get_logger().info("Force mode ON")
#                 else:
#                     self.get_logger().info("Force mode OFF")
