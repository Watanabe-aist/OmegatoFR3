...
class FrankaBridge(Node):  
    ...
    def __init__(self):
        ...
        self.max_joint_vel = 1.0 # [rad/s] まずは0.2〜0.5で様子見
    ...
    def _rate_limit(self, cmd, prev, dt, max_vel):
        if prev is None:
            return cmd
        max_step = max_vel * dt
        step = np.clip(cmd - prev, -max_step, max_step)
        return prev + step

    def left_franka_cmd_callback(self, msg):
        cmd = np.array(msg.position)
        cmd = self._rate_limit(cmd, self.prev_left_cmd, self.dt, self.max_joint_vel)
        self.prev_left_cmd = cmd
        #print("left cmd:", cmd)
        self.left_franka_cmd_pub.send_message(cmd)

    def right_franka_cmd_callback(self, msg):
        cmd = np.array(msg.position)
        cmd = self._rate_limit(cmd, self.prev_right_cmd, self.dt, self.max_joint_vel)
        self.prev_right_cmd = cmd
        #print("right cmd:", cmd)
        self.right_franka_cmd_pub.send_message(cmd)
    # def left_franka_cmd_callback(self, msg):
    #     self.left_franka_cmd_pub.send_message(np.array(msg.position))
    #     cmd = np.array(msg.position)
    #     self.get_logger().info(f"Left cmd: {cmd}")
    #     self.left_franka_cmd_pub.send_message(cmd)
